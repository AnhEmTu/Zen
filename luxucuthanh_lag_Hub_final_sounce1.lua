...
local ToggleBoneE = Tabs.Main:AddToggle("ToggleBoneE", {Title = "Auto BoneE", Default = false})
    ToggleBoneE:OnChanged(
        function(Value)
            _G.Bone = Value
        end
    )
    Options.ToggleBoneE:SetValue(false)
    local BoneCFrame = CFrame.new(-9515.75, 174.8521728515625, 6079.40625)
    spawn(
        function()
            while wait() do
...
local ToggleMonster = Tabs.Raid:AddToggle("ToggleMonster", {Title = "Đánh Quái (Đã Fix)", Default = false })
ToggleMonster:OnChanged(function(Value)
    Monster = Value
end)
Options.ToggleMonster:SetValue(false)
spawn(function()
    while wait() do
        if Monster then
            pcall(function()
                for i,v in pairs(game.Workspace.Enemies:GetDescendants()) do
                    if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                        repeat task.wait(0.2)
                            ...
                            v.Humanoid.Health = 0
                            v.HumanoidRootPart.CanCollide = false
                        until not Monster or not v.Parent or v.Humanoid.Health <= 0
                        bringmob = false
                    end
                end
            end)
...
Tabs.Misc:AddButton({
	...
	Description = "",
	Callback = function()
	loadstring(game:HttpGet('https://raw.githubusercontent.com/Dex-Bear/dexhubshowitems/refs/heads/main/showtiemsbearhub'))()
	end
})
